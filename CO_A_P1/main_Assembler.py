    ...
    if c not in opcode:
        ...
        if d in label_dict.keys():
            ...
            if c==" ":
                ...
                type_A_instructions=['add','sub','mul','xor','or','and','addf','subf']
...
def floating(imm):
    int_imm=int(imm)
    bin_int_imm=format(int_imm,'0b')
    deci_imm=imm-int_imm
    bin_deci_imm=''
    while deci_imm>0 and len(bin_deci_imm)!=5:
        deci_imm=deci_imm*2
        bit=int(deci_imm)
        bin_deci_imm=bin_deci_imm+str(bit)
        deci_imm=deci_imm-bit
    exponent=len(bin_int_imm)-1+3
    bin_temp=bin_int_imm[1:]+bin_deci_imm
    binary_out=format(exponent,'04b')[1:]+bin_temp[:5]
    return binary_out
...
def binary_gen(line,dec_dict,data_memory,reg_dict,opco_dict):
    ...
    if d in dec_dict.keys():
        ...
        Type_A=['add','sub','mul','xor','and','or','addf','subf']
        if c in Type_A:
        ...
        if c in Type_G:
            imm=int(b[-1][1:])
            opco_dict[c]+reg_dict[b[1]][0]+floating(imm)      
    if c=='mov':
        if '$' in b[-1]:
            imm=int(b[-1][1:])
            return opco_dict['mov']+'0'+reg_dict[b[1]][0]+format(imm,'07b')
    ...
    if c in Type_G:
        imm=int(b[-1][1:])
        return opco_dict[c]+reg_dict[b[1]][0]+floating(imm)      
...
opcode_list=['add','sub','mov','mov1','ld','st','mul','div','rs','ls','xor','or','and','not','cmp','jmp','jlt','jgt','je','addf','subf','movf','hlt']
opcode_dict={}
for i in range(len(opcode_list)-7):
    opcode_dict[opcode_list[i]]=format(i,"05b")
opcode_dict['jlt']='11100'
opcode_dict['jgt']='11101'
opcode_dict['je']='11111'
opcode_dict['addf']='10000'
opcode_dict['subf']='10001'
opcode_dict['movf']='10010'
